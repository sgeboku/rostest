...
class Turtlebot3Drive
{
 ...
 private:
  ...
  ros::Publisher det_state_pub_;

  // ROS Topic Subscribers
  ros::Subscriber laser_scan_sub_;
  ros::Subscriber odom_sub_;
  ros::Subscriber img_state_sub_;
  ...
  void updatecommandVelocityLinear(double linear);
  void updatecommandVelocityAngular(double angular);
  void laserScanMsgCallBack(const sensor_msgs::LaserScan::ConstPtr &msg);
  void odomMsgCallBack(const nav_msgs::Odometry::ConstPtr &msg);
  void imgStateCallBack(const std_msgs::Int32 &msg);
};
