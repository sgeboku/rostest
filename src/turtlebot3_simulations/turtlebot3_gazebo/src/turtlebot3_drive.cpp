...
bool Turtlebot3Drive::init()
{
  ...
  escape_range_       = 15.0 * DEG2RAD;
  check_forward_dist_ = 1.4;
  ...
  det_state_pub_   = nh_.advertise<std_msgs::Int32>("det_state", 10);
  ...
  img_state_sub_ = nh_.subscribe("img_state", 10, &Turtlebot3Drive::imgStateCallBack, this);

  return true;
}

void Turtlebot3Drive::imgStateCallBack(const std_msgs::Int32 &msg)
{
  check_state_ = msg.data; 
  printf("\n%d",check_state_);
}
...
void Turtlebot3Drive::updatecommandVelocityLinear(double linear)
{
  geometry_msgs::Twist cmd_vel;
...
void Turtlebot3Drive::updatecommandVelocityAngular(double angular)
{
  geometry_msgs::Twist cmd_vel;
...
void Turtlebot3Drive::updateState(int state)
{
  std_msgs::Int32 st;
  st.data = state;
  det_state_pub_.publish(st);
}
...
bool Turtlebot3Drive::controlLoop()
{
  ...
  switch(turtlebot3_state_num)
  {
    ...
    case TB3_DRIVE_FORWARD:
      updatecommandVelocityLinear(LINEAR_VELOCITY);
...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    ...
    // turtlebot3_drive.updateState(5);
    ros::spinOnce();
    loop_rate.sleep();
  }
